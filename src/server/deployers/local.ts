...
*(populated through experience)*`;
  ...
  async teardown(result: DeployResult, log: LogCallback): Promise<void> {
    ...
    // Use the actual container name to derive volume name instead of recomputing from config
    // This prevents volume name doubling when instance config is reconstructed without saved .env
    const containerId = result.containerId || containerName(result.config);
    const vol = `${containerId}-data`.toLowerCase();
