...
function buildRunArgs(
  ...
  const hasSidecars = useLitellm || useTkz;
...
*(populated through experience)*`;
    ...
    const runArgs = buildRunArgs(config, runtime, name, port, litellmMasterKey, tokenizerEnv);
    ...
    log("");
    log("=== Container Info ===");
    const hasSidecars = useProxy || useTokenizer_;
    if (hasSidecars) {
      const isPodman = runtime === "podman";
      if (isPodman) {
      ...
      if (useTokenizer_) {
        log(`Tokenizer container: ${tkzContainerName(name)}`);
      }
      log("");
      log("Useful commands:");
      if (isPodman) {
      ...
      if (useTokenizer_) {
        log(`  ${runtime} logs ${tkzContainerName(name)}  # Tokenizer proxy logs`);
      }
    } else {
      log(`Container: ${name}`);
      log("");
  ...
  async start(result: DeployResult, log: LogCallback): Promise<DeployResult> {
    ...
    log(`Starting OpenClaw container: ${name}`);
    const runArgs = buildRunArgs(result.config, runtime, name, port, litellmMasterKey, tokenizerEnv);
    const run = await runCommand(runtime, runArgs, log);
    if (run.code !== 0) {
      throw new Error("Failed to start container");
    }
